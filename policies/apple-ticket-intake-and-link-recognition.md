...
|---|---|---|
| "Failed to Join Organization — There was an issue with your unique link." | In-app, after tapping the link | Most often a **sign-in mismatch**: app signed into a different account than the Wellness registration. Fill the registered address into `Support GlobalAccesLoginEmail` (compare the report's `Email:` line against the registration), or `Support GlobalAccessLoginSIWA` for privaterelay accounts (#309701, #309645). |
